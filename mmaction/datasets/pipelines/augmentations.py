...
class RandomBrightness:
    def __init__(self, brightness_range=(0.75, 1.25)):
        self.brightness_range = brightness_range

    def __call__(self, results):
        brightness = np.random.uniform(self.brightness_range[0], self.brightness_range[1])
        results['imgs'] = [img * brightness for img in results['imgs']]
        return results
...
class RandomContrast:
    ...
    def __call__(self, results):
        ...
        # Assuming results['imgs'] is a list of NumPy arrays with shape (height, width, channels)
        for i in range(len(results['imgs'])):
            img = results['imgs'][i].astype(np.float32)
            img *= contrast_factor
            np.clip(img, 0, 255, out=img)
            results['imgs'][i] = img.astype(np.uint8)
...
class RandomGaussianBlur:
    def __init__(self, blur_probability=0.5, ksize=(5, 5), sigma=1.0):  # Added sigma here
        self.blur_probability = blur_probability
        self.ksize = ksize
        self.sigma = sigma  # Added sigma here
