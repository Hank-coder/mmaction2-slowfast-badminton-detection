...
Change this!!!
"""
custom_classes = [1, 2, 3, 4, 5]  # class
...
model = dict(
    ...
    roi_head=dict(
        ...
        bbox_head=dict(
            ...
            multilabel=True,
            topk=(1, 1),
            dropout_ratio=0.30)),  # smaller dropout since no much data 快收敛
    train_cfg=dict(
        rcnn=dict(
            assigner=dict(
...
train_pipeline = [
    ...
    dict(type='RandomBrightness', brightness_range=(0.75, 1.25)),  # new
    dict(type='RandomContrast', contrast_range=(0.8, 1.2)),  # new
    dict(type='RandomGaussianBlur', sigma=0.15),  # new
...
optimizer = dict(type='SGD', lr=0.1125/8, momentum=0.9, weight_decay=0.0001)
# this lr is used for 8 gpus
# divide 8 to adapt one gpu, weight_decay=0.0001
